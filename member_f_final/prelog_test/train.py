...
# === ONLINE_BOUTIQUE_BATCH_HELPER_START ===
def label_to_id(x):
    if hasattr(x, "item"):
        x = x.item()
    ...
    if isinstance(x, str):
        value = x.strip().lower()
        if value in {"faulty", "fault", "abnormal", "anomaly", "1", "true"}:
            return 1
        if value in {"clean normal", "normal", "clean", "0", "false"}:
            return 0
        return 0
    return int(x)
...
def move_batch_to_device(batch, device):
    ...
    label_values = None
    for key in ("guid", "label", "labels"):
        if key in batch:
            label_values = batch[key]
            break

    if label_values is not None:
        if hasattr(label_values, "detach"):
            values = label_values.detach().cpu().tolist()
        elif isinstance(label_values, (str, int, float)):
            values = [label_values]
        else:
            values = list(label_values)

        label_tensor = torch.tensor(
            [label_to_id(x) for x in values],
            dtype=torch.long,
            device=device
        )
        fixed_batch["guid"] = label_tensor
        fixed_batch["label"] = label_tensor

    return fixed_batch
# === ONLINE_BOUTIQUE_BATCH_HELPER_END ===
...
def train(tokenizer, model, template, max_length, WrapperClass, args):
    ...
    criterion = torch.nn.CrossEntropyLoss(weight=torch.tensor([1.0, 1.1], dtype=torch.float, device=device))
